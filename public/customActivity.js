define(['postmonger'], function (Postmonger) {
  ...
  // WHEN THE VONAGE MMS ICON IS CLICKED
  function onInitActivity(data) {
    console.log('🟡 onInitActivity data:', data.arguments);
    if (data) {
      activityData = data;
      activityName = 'MMS Activity'; // activityData.name;
    }
    ...
    $.each(inArguments, function (index, inArgument) {
      $.each(inArgument, function (key, val) {
        ...
        if (key === 'imageUrl') {
          console.log('imageUrl val:', val);
          imageUrl = val;
          $('#imageUrl').val(val);
        }

        if (key === 'imageCaption') {
          console.log('imageCaption val:', val);
          imageCaption = val;
          $('#imageCaption').val(val);
        }
  ...
  function onSave() {
    ...
    activityData.name = 'Vonage MMS'; // getActivityName();
  ...
  function configureInArguments() {
    ...
    var inArguments = [];
    inArguments.push({ imageUrl: imageUrl });
    inArguments.push({ imageCaption: imageCaption });
  ...
  function getActivityName() {
    if (isEmptyString(activityName)) {
      activityName = 'Vonage MMS'; // constructActivityName();
    }
    return activityName;
  }
